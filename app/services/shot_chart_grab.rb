class ShotChartGrab
  ...
  def get_shot_charts
    ...
    games.each do |game|
      game.dressed_players.each do |player|
        shot_chart_json = HTTP.get(link_builder(player, game)).parse

        shot_chart_json['resultSets'][0]['rowSet'].each do |shot_chart|
          shot_chart_data = grab_specific_shot_chart_data(shot_chart)
          new_shot_chart = ShotChart.find_by(nba_game_id: shot_chart_data[:nba_game_id],
            nba_player_id: player.nba_player_id, period: shot_chart_data[:period],
  ...
  def grab_specific_shot_chart_data(stats)
    ...
    shot_chart_data[:nba_game_id] = stats[1]
    shot_chart_data[:nba_grid_type] = stats[0]
    shot_chart_data[:nba_player_id] = stats[3]
    shot_chart_data[:nba_team_id] = stats[5]
