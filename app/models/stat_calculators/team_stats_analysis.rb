module StatCalculators
  class TeamStatsAnalysis
    ...
    def game_stats
      game_stats = {}
      games = Game.where("date BETWEEN ? AND ?", @start_date, @end_date).where(completed?: true)
    ...
    def game_by_game_possessions
      ...
      self.game_stats.each do |k,v|
        game = Game.includes(:home_team, :away_team).where(nba_id: k, completed?: true)[0]
        game_possessions = possessions_formula(v)
        game_by_game_possessions[k] = { game.home_team[:nba_id] => game_possessions[:home],
                                          game.away_team[:nba_id] => game_possessions[:away]}
      end
    ...
    def game_by_game_points
      ...
      self.game_stats.each do |k,v|
        game = Game.includes(:home_team, :away_team).where(completed?: true, nba_id: k)[0]
        game_by_game_points[k] = { game.home_team[:nba_id] => points_formula(v["home"]),
                                    game.away_team[:nba_id] => points_formula(v["away"])}
      end
    ...
    def team_total_possessions
      ...
      @teams.each do |team|
        games = team.games.select { |game| game.completed? && game.date >= @start_date && game.date <= @end_date }

        if !games.empty?
          total_possessions = games.map do |game|
            game_possessions[game.nba_id].values.inject(:+) / 2.0
          end.inject(:+)

          team_total_possessions[team.nba_id] = total_possessions
        end
      end
      ...
      @teams.each do |team|
        ...
        if !games.empty?
          total_points = games.map do |game|
            game_points[game.nba_id][team.nba_id]
          end.inject(:+)

          team_total_points[team.nba_id] = total_points
        end
      end
      ...
      @teams.each do |team|
        ...
        if !games.empty?
          total_points_allowed = games.map do |game|
            game_points[game.nba_id].values.inject(:+) - game_points[game.nba_id][team.nba_id]
          end.inject(:+)

          team_total_points_allowed[team.nba_id] = total_points_allowed
        end
      end
    ...
    def team_total_minutes
      ...
      @teams.each do |team|
        ...
        if !games.empty?
          ...
          team_total_minutes[team.nba_id] = total_minutes
        end
      end
